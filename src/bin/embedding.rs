...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let embeddings: Vec<(WordDefinition, rig::OneOrMany<rig::embeddings::Embedding>)> = EmbeddingsBuilder::new(embedding_model.clone())
        .documents(vec![
            WordDefinition {
                id: "doc0".to_string(),
    ...
    let rag_agent = openai_client.agent(&config.model)
        .preamble("
